function VisualizationJS() {
    ...
    const loadMap = function (mapId) {
        ...
        mapData.forEach(row => {
            ...
            const m = L.marker([lat, lng], { icon: brandIcon, ref: first_ref })
                .addTo(map)
                .bindPopup(popupHtml);
    ...
    function buildFilteredWordcloudData(tabTag) {
        ...
        return (entityData || [])
            ...
            .map(a => ({
                name: a.text,
                value: Math.floor(Math.random() * a.count) + a.count,
                ref: a.first_ref,
    ...
    const getStickyHeaderOffset = function (container, transcriptTab) {
        let offset = 0;
        ...
        const selectors = [
            ...
            '.top-details:visible',
            '.top-meta:visible',
            '.transcript-sticky-header:visible'
        ];

        let $header = $();

        for (let i = 0; i < selectors.length; i++) {
            $header = $container.find(selectors[i]).first();
            if ($header.length) break;
        }

        if (!$header.length) {
            for (let i = 0; i < selectors.length; i++) {
                $header = $tab.find(selectors[i]).first();
                if ($header.length) break;
            }
        }

        if ($header.length) {
            offset = $header.outerHeight(true) || 0;
        }
    ...
    const scrollToTranscript = function (container, transcriptTab, ref) {
        ...
        setTimeout(function () {
            ...
            const stickyOffset = getStickyHeaderOffset(container, transcriptTab);
    ...
    const browserTab = function () {
        ...
        $('#sortDropdown1, #sortDropdown2').on('change', function () {
            ...
            let colIndex = 0;
            if (col === 'type') colIndex = 1;
            if (col === 'name') colIndex = 2;
