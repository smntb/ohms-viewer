function Viewer() {
    ...
    this.initialize = function () {
        ...
        $('#custom-tabs-right').on('tabsactivate', function () {
            toggleLayersBtns();
        });
    ...
    const toggleLayersBtns = function () {
        let activeRightTab = $('#custom-tabs-right .ui-tabs-nav li.ui-tabs-active a').attr('href');

        if (!activeRightTab) {
            activeRightTab = $('#custom-tabs-right .ui-tabs-nav li.ui-tabs-selected.ui-state-active a').attr('href');
        }

        if (activeRightTab === '#transcript-tab-2') {
            $('.right-side .layers-btns').show();
        } else {
            $('.right-side .layers-btns').hide();
        }
    };
...
function IndexJS() {
    ...
    const bindEvents = function () {
        ...
        $('.footnoteLink').click(function (e) {
            ...
            setTimeout(function () {
                let scrollTo = $(transcriptTab + " " + linkTo);
                container.animate({
    ...
    const switchIndexToTranscript = function () {
        $('.mapIndexTranscript').click(function () {
            ...
            setTimeout(function () {
                let scrollTo = $(transcriptTab + ">.transcript-panel>.info_trans_" + id);
                container.animate({
                    scrollTop: scrollTo.offset().top - container.offset().top + container.scrollTop()
                });
            }, 250);
                ...
                $.getJSON('viewer.php?action=index' + external + '&cachefile=' + cachefile + '&kw=' + kw + (isTranslate ? '&translate=1' : ''), function (data) {
                    ...
                    if (data.matches.length === 0) {
                        ...
                        $('a.index-search-result').on('click', function (e) {
                            ...
                            lineTarget = $(e.target);
                            let linenum = lineTarget.data("linenum");
                ...
                if (data.matches.length === 0) {
                    ...
                    $.each(data.matches, function (key, val) {
                        ...
                        if (/^((?!chrome|android).)*safari/i.test(navigator.userAgent) || navigator.userAgent.search("Firefox")) {
                            re = new RegExp("(?![^<>]*(([\/\"']|]]|\\b)>))(" + preg_quote(data.keyword) + ')', 'gi');
                        } else {
                            re = new RegExp('(?<!</?[^>]*|&[^;]*)(' + preg_quote(data.keyword) + ')', 'gi');
                        }
